...
現在のバージョン: **v1.1.9 - Coordinate Browser filter help**
## 1. このプログラムの目的
...
| Atom filter | `3`, `3,12`, `3-12` | atom index で絞り込みます。`3,12` は 3 または 12、`3-12` は 3 と 12 の両方を含む座標を表示します。 |
| Contains element | `Co` | Co を含む座標だけを見る |
| Text filter | `C4-C11`, `CC`, `ZnO`, `str` | atom label、VEDA label、raw label、元行を文字列検索します。 |
...
#### Atom filter と Text filter

`Atom filter` は atom index による絞り込みです。`3` は atom 3 を含む座標、`3,12` は atom 3 または atom 12 を含む座標、`3-12` は atom 3 と atom 12 の両方を含む座標を表示します。全角ハイフンや en dash を用いた `3－12`, `3–12` も同じ意味で扱います。

`Text filter` は、表示される atom label、VEDA label、raw label、元行に対する自由文字列検索です。`C4-C11`, `CC`, `ZnO`, `str` などで絞り込めます。

金属–配位子伸縮を探す場合は、まず以下のように絞ると便利です。

```text
Group = STRE
Atom filter = 金属原子の atom index
```
...
## 16. `s:81`, `k:128`, `v:81` 形式の target 指定

v1.1.3 以降、target 座標は `coord_id` だけではなく、coordinate set と coord_id を組み合わせた `code:coord_id` 形式で管理します。

例:

```text
s:81   standard 座標セットの coord_id 81
k:128  alternative_k 座標セットの coord_id 128
v:81   alternative_v 座標セットの coord_id 81
```

これは、同じ numeric `coord_id` が coordinate set によって異なる内部座標を表す場合があるためです。たとえば `s:81` が C4-C5 を表し、`v:81` が C4-C11 を表す場合、従来の `81` だけの指定では両者を区別できません。

Coordinate Browser から `Add selected to target` を押した場合、新しい形式で自動的に target に追加されます。従来互換のため `81` のような数値だけの指定も読み込めますが、新しい解析では `s:81`, `k:128`, `v:81` のように明示することを推奨します。

`Include alternative coordinate sets (k/v)` を ON にした場合、standard、alternative_k、alternative_v の出力が別々に生成されます。standard の結果を残したまま alternative 座標の結果を確認できます。

なお、`v` は `ALTERNATIVE2` に対応する第2の alternative coordinate set です。必ずしも `k` の拡張版という意味ではなく、VEDA/DD2 が与える別解釈として扱うのが安全です。


## Combined target outputs（v1.1.3）

化学的に対応する内部座標が同じ VEDA 座標セット内にそろわない場合は、`s:83` と `k:126` のような code-qualified target reference を同じ target set に登録できます。Run Analysis タブで **Combined target outputs (allow mixed s/k/v targets)** を有効にすると、以下の combined 出力が作成されます。
...
combined 出力では、選択した `target_ref` をそのまま用いて合計 PED を計算します。したがって、standard と alternative の座標を混在させても、どの座標由来の寄与かを追跡できます。同じ物理的結合を `k:126` と `v:76` のように重複登録すると二重計上になり得るため、通常は一つだけ選択してください。
